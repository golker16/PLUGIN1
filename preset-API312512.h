...
struct Preset_API312512
{
    ...
    static inline float process (State& s, float xIn, const plugin::Knobs& k) noexcept
    {
        ...
        // Knob smoothing (API: más directo)
        {
            const Real aK = alphaFromMsT ((Real) 9.0, sr);
            // ✅ 3 args SIEMPRE (tu error era que esto estaba con 2 args en tu archivo real)
        ...
        // 1) Detector + env + sag (API rápido y menor)
        Real xm = 0.0;
        {
            Real lfInst = 0.0;
            {
        ...
        low = transformerHystADAA_HQ(
            ...
            low, env01, sag01, sr,
            (Real) 72.0 * trimFc * trimTrafo,
            (Real) 0.16,
            (Real) 1.85 * trimDrive,
            (Real) 0.007 * trimBias,
            (Real) (0.16 * (0.95 + 0.10 * trimTrafo))
        );
        ...
        // Punch Bus paralelo
        {
            ...
            Real fcList[State::kDiffN] = {
                (Real) 520.0, (Real) 980.0, (Real) 1700.0, (Real) 2900.0
            };
            const Real smearMul =
...
private:
    ...
    static constexpr float kInCal  = 7.943282347242814f;
    static constexpr float kOutCal = 0.12589254117941673f;
